...
def build_zipapp():
    ...
    zipapp.create_archive(SRC, pyz, interpreter='/usr/bin/env python3')

    dst_plugins = os.path.join(out_dir, 'plugins')
    if os.path.exists(dst_plugins):
        shutil.rmtree(dst_plugins)
    shutil.copytree(PLUGINS, dst_plugins)
...
def main():
    parser = argparse.ArgumentParser(description='Build pyInstantShooter distributables.')
    parser.add_argument(
        'target',
        nargs='?',
